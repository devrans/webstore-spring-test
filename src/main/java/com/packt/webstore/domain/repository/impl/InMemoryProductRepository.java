...
public class InMemoryProductRepository implements ProductRepository{
	...
	public List<Product> getProductsByManufacturer(String manufacturer) {
		...
		productsByManufacturer = listOfProducts.stream().filter(x -> manufacturer.equals(x.getManufacturer())).collect(Collectors.toCollection(ArrayList::new));
		
		//	productsByManufacturer.forEach(System.out::println);
//		for (Product product : listOfProducts) {
...
//		}
	...
	public List<Product> getProductsByPriceFilter(BigDecimal low, BigDecimal high) {
		List <Product> productsByPrices = new ArrayList<Product>();
		for (Product product : listOfProducts) {
			if(product.getUnitPrice().intValue() >= low.intValue() && product.getUnitPrice().intValue() <= high.intValue() ) {
